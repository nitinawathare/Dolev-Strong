...
//The leader sends the message initially.
void DolevS::initialize()
{
    // Node 0 (leader) sends the first message
    if (getIndex() == 0) {
        f = par("f");
        n = par("n");
        ...
        int i = 0;
        int gateSize = gate(gateName, 0)->size();
        //In round 0 honest leader broadcasts a message.
        if(f==0){
            do {
                destGate = gate(gateName, i++);
                cGate *nextGate = destGate->getNextGate();
...
//Handle all the messages received at the node.
void DolevS::handleMessage(cMessage *msg)
{
    ...
    //For round 1...f, if an honest node receives the message from other nodes, they broadcast it to all their neighbors after adding their signature.
    else if(getIndex()>=f){   
        ...
        //Honest nodes forward the message if they are convinced of the value included in the message
...
//Checks whether the current node instance is convinced 
//of the value included in the message
bool DolevS::convincedOfValue(DolevStrongMsg *msg){
    //The message is signed by the sender (leader) and t-1 other distinct nodes.
...
//Leader generates the message in round 0
...
//Honest nodes call this method to broadcast the message.
...
//At round f+1 (when the simulation end), all the honest nodes print the values they are convinced of.
void DolevS::finish(){
